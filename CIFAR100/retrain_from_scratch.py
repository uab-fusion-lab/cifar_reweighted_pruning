...
def masked_retrain(criterion, optimizer, scheduler):
    ...
    model_record_name = "./model_reweighted/temp3.pt"
        ...
        if args.rew:
            ...
            for j in range(len(layers)):
                ...
                if args.sparsity_type == "irregular":
                    ...
                    l1_loss = l1_loss + 1.5e-3 * torch.sum(rew * torch.norm(torch.norm(conv_layer, dim=1), dim=[1, 2]))
...
def test(model, criterion, test_loader):
    ...
    return (nat_losses.avg + adv_losses.avg)/2, adv_acc
